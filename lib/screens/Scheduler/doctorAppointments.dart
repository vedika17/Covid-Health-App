...
class _DoctorAppointmentsState extends State<DoctorAppointments> {
  ...
  Widget _listViewRequestedAppointments() {
    return FutureBuilder(
      future: DB().getRequetedAppointments(widget.phone),
  ...
  Widget _listViewUpcomingAppointments() {
    return FutureBuilder(
      future: DB().getUpcomingAppointments(widget.phone),
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.data == null) {
          return Loading();
        } else if (snapshot.data.length == 0) {
  ...
  Widget _listViewPastAppoitments() {
    return FutureBuilder(
      future: DB().getPastAppointmentsDoctor(widget.phone),
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.data == null) {
          return Loading();
        } else if (snapshot.data.length == 0) {
