...
class RegisterDoctor extends StatefulWidget {
  final String email;
  final String phone;
  RegisterDoctor(this.email, this.phone);
  @override
  _RegisterDoctorState createState() => _RegisterDoctorState(email, phone);
}
...
class _RegisterDoctorState extends State<RegisterDoctor> {
  ...
  String phone;
  String email;
  String fName, lName, degree, pass, cPass;
  int packageFee;
  int fee;
  int followUpfee;
  String upi, account, ifsc, pkg, packageDays;
  ...
  _RegisterDoctorState(this.email, this.phone);
  Widget _buildFName() {
    return Theme(
  ...
  // Text("Profile Photo Selected", style: TextStyle(color: Colors.green, fontSize: 20, fontWeight: FontWeight.bold),),
                    ...
                    onPressed: () async {
                      ...
                      if (_profile != null) {
                        ...
                        StorageReference pictureFolderRef =
                            rootRef.child("ProfileImages").child(email);
                        pictureFolderRef
                            .putFile(_profile)
                            .onComplete
                            .then((storageTask) async {
                          String link = await storageTask.ref.getDownloadURL();
                          d.profileImageLink = link;
                        });
                      }
                      // if (_image == null) {
                      //   Widget okButton = FlatButton(
                      ...
                      //   AlertDialog alert = AlertDialog(
                      //     title: Text("Error"),
                      //     content: Text("Please enter your document"),
                      //     actions: [
                      //       okButton,
                      //     ],
                      //   );
